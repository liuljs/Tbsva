...
let methodz = '02', setUpz = '',
    insert = `
        <a class="btn btn-primary" href="./chairman_01.html">
            <i class="fad fa-file-plus"></i> 新增理事長
        </a>
    `;
// 宣告要帶入的欄位
...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        // 顯示現任
        if (data[i].first == 1) {
            topz = '<span class="text-primary">現任</span>';
        } else {
            topz = '<span> - </span>';
        };
        // 顯示狀態
        ...
        html += `
            <tr data-numz="${data[i].directorIntroductionId}">
                ...
                <td data-title="副標題" class="fieldLefts">
                    <div class="fullFields">
                        <p class="mb-0 abridged1">${data[i].subtitle}</p>
                    </div>
                </td>
                <td data-title="現任" class="text-center">
    ...
    $('.btnEditz').on('click', function (e) {
        ...
        if (idz) {
            ...
            let chaNum = $(this).parents('tr').data('numz');
            localStorage.setItem("chaNum", chaNum);
            let numz = localStorage.getItem("chaNum");
            if (numz) { // 確認有將消息編號存入 Storage
                location.href = "./chairman_03.html"; // 跳轉至編輯頁面
            };
        };
    });
    // Delete
...
$().ready(function () {
    // 清除 localStorag 中可能留著的 chaNum
    if (localStorage.getItem('chaNum')) {
        localStorage.removeItem('chaNum');
    };
    let dataObj = {
        "Methods": "GET",
        "APIs": URL,
