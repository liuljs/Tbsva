...
let methodz = '02', setUpz = '',
    insert = `
        <a class="btn btn-primary" href="./movie_01.html">
            <i class="fad fa-file-plus"></i> 新增影片
        </a>
    `;
// 宣告要帶入的欄位
let list = $('.list');
...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        // 狀態
        if (data[i].enabled == "1") {
            disStatus = '<span class="text-success">開啟</span>';
        } else {
            disStatus = '<span class="text-danger">關閉</span>';
        };
        html += `
            <tr data-numz="${data[i].videoId}">
                <td data-title="編號" class="text-center">
                    <div>${data[i].id}</div>
                </td>
                <td data-title="建立日期" class="fieldLefts">
                    <div class="fullFields">${data[i].createDate.split(' ')[0]}</div>
                </td>
                <td data-title="標題">
                    <div class="fullFields" class="fieldLefts">
                        <p class="mb-0 abridged1">${data[i].name}</p>
                    </div>
                </td>
                <td data-title="類別" class="text-center fieldLefts" data-numz="${data[i].videovideoCategory}">
                    <div class="fullFields">
                        <p class="mb-0 classify">${movCategoryDisplay(data[i].videoCategory)}</p>
                    </div>
                </td>
                <td data-title="狀態" class="text-center"><span data-statusz="${data[i].enabled}">${disStatus}</span></td>
                <td data-title="設定">
    ...
    $('.btnEditz').on('click', function (e) {
        ...
        if (idz) {
            ...
            let movNum = $(this).parents('tr').data('numz');
            localStorage.setItem("movNum", movNum);
            let numz = localStorage.getItem("movNum");
            if (numz) { // 確認有將消息編號存入 Storage
                location.href = "./movie_03.html"; // 跳轉至編輯頁面
            };
        };
    });
    // Delete
...
$().ready(function () {
    // 清除 localStorag 中可能留著的 movNum
    if (localStorage.getItem('movNum')) {
        localStorage.removeItem('movNum');
    };
    //
    let dataObj = {
        "Methods": "GET",
    ...
    btnSearchz.on('click', function (e) {
        ...
        if (check == true) {
            ...
            // 條件
            if (clsNum !== "" && clsName !== "") {
                dataObj.CONNECTs = `${CONNECT}?videoCategory=${clsNum}&BuyerName=${clsName}`;
            } else if (clsNum !== "" || clsName !== "") {
                dataObj.CONNECTs = `${CONNECT}?videoCategory=${clsNum !== "" ? clsNum : ""}&BuyerName=${clsName !== "" ? clsName : ""}`;
            };
                    ...
                    paginations.unbind('click').on('click', 'li', function (e) {
                        ...
                        if (isNaN(num)) {
                            if (!$(this).hasClass("disabled")) {
                                ...
                                pageRcd = pageRcd, dataObj.Sends.page = pageRcd;
                                // 2. 條件
                                dataObj.QUERYs = `${CONNECT}?count=${listSize}&page=${pageRcd}&videoCategory=${clsNum}&BuyerName=${clsName}`;
                                // 3. 取得點擊頁碼後要呈現的內容
                                getPageDatas(dataObj).then(res => {
                                    // DO SOMETHING
                                    if (res !== null) {
