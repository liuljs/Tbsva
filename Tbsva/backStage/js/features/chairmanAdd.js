// 宣告要帶入的欄位
let topz = $('.topz'), titlez = $('.titlez'), subTitlez = $('.subTitlez'), imgFilez = $('.imgFilez'), titleImagez = $('.titleImagez'), titleSamplez = $('.titleSamplez'), briefz = $('.briefz');

let btnResetz = $('.btnResetz'), btnAddz = $('.btnAddz'), btnReturnz = $('.btnReturnz');
let enabled = 1; // 預設為顯示 
CONNECT = 'directorIntroduction';
// 驗證
function dataUpdateCheck(aId, title, sub, file, brief, content) {
    if (aId.trim() === '') {
        title.focus();
...
$().ready(function () {
    ...
    btnAddz.on('click', function (e) {
        e.preventDefault();
        // 驗證
        dataUpdateCheck(idz, titlez, subTitlez, titleImagez, briefz, quill);
        if (check == true) {
            // 取得圖片的名稱包成陣列
