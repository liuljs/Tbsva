...
let startDatez = $('.startDatez'), endDatez = $('.endDatez');

let btnResetz = $('.btnResetz'), btnAddz = $('.btnAddz'), btnReturnz = $('.btnReturnz');
let enabled = 1; // 預設為顯示 
CONNECT = 'news';
// 驗證
function dataUpdateCheck(aId, title, file, contentz) {
    if (aId.trim() === '') {
        title.focus();
...
$().ready(function () {
    ...
    btnResetz.on('click', function (e) {
        ...
        if (confirm("您確定要將目前編輯的內容全部清空嗎？")) {
            ...
            // 回復預設
            titleSamplez.html(`
                <img src="./img/samples/sample500x500.jpg">
            `);
            // 清空上下架時間
            startDatez.val(''), endDatez.val('');
            // 清空預覽圖
            picImagez.val('');
            // 回復預設
    ...
    btnAddz.on('click', function (e) {
        e.preventDefault();
        // 驗證
        dataUpdateCheck(idz, titlez, titleImagez, quill);
        if (check == true) {
            // 取得圖片的名稱包成陣列
