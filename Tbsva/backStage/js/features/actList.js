...
let methodz = '02', setUpz = '',
    insert = `
        <a class="btn btn-primary" href="./activity_02.html">
            <i class="fad fa-file-plus"></i> 新增活動
        </a>
    `;
// 宣告要帶入的欄位
let list = $('.list');
...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        html += `
            <tr data-numz="${data[i].activityId}">
    ...
    $('.btnEditz').on('click', function (e) {
        ...
        if (idz) {
            ...
            let actNum = $(this).parents('tr').data('numz');
            localStorage.setItem("actNum", actNum);
            let numz = localStorage.getItem("actNum");
            if (numz) { // 確認有將消息編號存入 Storage
                location.href = "./activity_04.html"; // 跳轉至編輯頁面
            };
        };
    });
    // Delete
...
$().ready(function () {
    // 清除 localStorag 中可能留著的 actNum
    if (localStorage.getItem('actNum')) {
        localStorage.removeItem('actNum');
    };
    // 類別
    let clsObj = {
        "Methods": "GET",
        "APIs": URL,
        "CONNECTs": `activity/category`,
        "QUERYs": "",
        "Counts": listSize,
        "Sends": "",
    };
