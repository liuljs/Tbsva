...
CONNECT = "articleContent", TABCONNECT = "articleCategory";
// 接收資料，做渲染、處理
function process(data) {
    html = "";
    for (let i = 0; i < data.length; i++) {
        html += `
            <a href="./supPagins.html?id=${data[i].id}" class="items itemz">
                <div class="parts border">
                    <div class="images">
                    <img src="./img/elements/samples/sample500x600.jpg">
                        <img class="cover imagez" src="${data[i].imageURL}?supervisor${paramRandoms()}">
                    </div>
                </div>
                <div class="parts border">
                    <div class="heads">
                        ...
                        <div class="parts border">
                            <p class="abridged1" title="${data[i].subtitle}">${data[i].subtitle}</p>
                        </div>
                    </div>
                    <div class="btns">
        ...
        tabz.find('li').on('click', function (e) {
            ...
            if (clsNum !== clsRcd) {
                ...
                dataObj = {
                    "Methods": "GET",
                    "APIs": URL,
                    "CONNECTs": `${CONNECT}?articleCategoryId=${clsNum}`,
                    "QUERYs": `${CONNECT}?articleCategoryId=${clsNum}&count=${listSize}&page=${current}`,
                    "Counts": listSize,
                    "Sends": "",
                };
                    ...
                    paginations.unbind('click').on('click', 'li', function (e) {
                        ...
                        if (isNaN(num)) {
                            if (!$(this).hasClass("disabled")) {
                                ...
                                pageRcd = pageRcd;
                                // 2. 條件
                                dataObj.QUERYs = `${CONNECT}?articleCategoryId=${clsRcd}&count=${listSize}&page=${pageRcd}`;
                                // 3. 取得點擊頁碼後要呈現的內容
                                getPageDatas(dataObj).then(res => {
                                    // DOSOMETHING
                                    if (res !== null) {
                            ...
                            if (clsNum !== clsRcd) {
                                ...
                                // 2. 條件
                                dataObj.QUERYs = `${CONNECT}?articleCategoryId=${clsRcd}&count=${listSize}&page=${current}`;
                                ...
                                if (num !== pageRcd) { // 如果不是點同一頁碼的話
                                    ...
                                    pageRcd = num;
                                    // 2. 條件
                                    dataObj.QUERYs = `${CONNECT}?articleCategoryId=${clsNum}&count=${listSize}&page=${num}`;
